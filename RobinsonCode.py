...
# # this is a list that stores the msot visited nest in each trial and the corresponding threshold from MC simulation
# most_Visited = []
...
def RobinsonCode(n, quals, probs, threshold_mean, threshold_stddev, qual_stddev, time_means, time_stddevs, ToPlot, quora):
    ...
    # times = np.zeros(n) #represents row vector of times to the nest acceptance
    
    # # quora: 1 * m matrix of quorum times for each nest site
    # quora = np.zeros([10, nestNum])
    # preqtimes = np.zeros(n)
    # preqdiscovers = np.zeros([nestNum, n])
    # preqvisits = np.zeros([nestNum, n])
    # preqaccepts = np.zeros([n], dtype=int)
    ...
    for i in range(n): # note that Python indexing is from 0, whereas MATLAB is from 1
        ...
        while Ants[i]['selected'] == 0:
            ...
            # uncomment the following code block if you wish to use quorum sensing
            # if times[i] > quora[1, newsite]:
            #     # if past pre-quorum period, then no new nests can be added
            #   preqtimes[i] = float('nan')
            #   preqaccepts[i] = preqaccepts[i]

            #   if preqdiscovers[newsite, i] == 0:
            #       preqdiscovers[newsite, i] = float('nan')
    ...
    # print(np.bincount(accepts).argmax(), thresh, perceivedQuality)
    # print(accepts)
    # max_Visited.append(perceivedQuality)
    # print(max_Visited)

    return current_time, discovers, visits, accepts, Ants, rnd_seed, [], [], [], []
    # preqtimes, preqdiscovers, preqvisits, preqaccepts
